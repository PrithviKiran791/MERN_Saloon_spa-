...
var Engine = class {
	...
	async load(params) {
		const id = params.id ?? params.element?.id ?? `tsparticles${Math.floor(getRandom() * loadRandomFactor)}`, { index, url } = params, options = url ? await getDataFromUrl({
			fallback: params.options,
			url,
			index
		}) : params.options, currentOptions = itemFromSingleOrMultiple(options, index), { items } = this, oldIndex = items.findIndex((v) => v.id.description === id), newItem = new Container(this, id, currentOptions);
...
//#endregion
export { AnimationStatus, Circle, CollisionMode, DestroyType, DivType, InteractorType, OptionsColor, OutMode, OutModeDirection, ParticleOutType, Rectangle, ValueWithRandom, Vector, calculateBounds, circleBounce, circleBounceDataFromParticle, clamp, colorMix, deepExtend, degToRad, divMode, divModeExecute, drawLine, errorPrefix, executeOnSingleOrMultiple, getDistance, getDistances, getHslAnimationFromHsl, getLinkColor, getLinkRandomColor, getLogger, getRandom, getRangeMax, getRangeValue, getStyleFromHsl, getStyleFromRgb, hslToRgb, hslaToRgba, initParticleNumericAnimationValue, isArray, isDivModeEnabled, isInArray, isNull, isObject, isPointInside, isSsr, itemFromArray, itemFromSingleOrMultiple, loadFont, millisecondsToSeconds, mouseLeaveEvent, mouseMoveEvent, parseAlpha, percentDenominator, randomInRange, rangeColorToHsl, rangeColorToRgb, rectBounce, rgbToHsl, setRangeValue, tsParticles, updateAnimation, updateColor };
//# sourceMappingURL=browser-DyR09nk6.js.map