import { AnimationStatus, Circle, CollisionMode, DestroyType, DivType, InteractorType, OptionsColor, OutMode, OutModeDirection, ParticleOutType, Rectangle, ValueWithRandom, Vector, calculateBounds, circleBounce, circleBounceDataFromParticle, clamp, colorMix, deepExtend, degToRad, divMode, divModeExecute, drawLine, errorPrefix, executeOnSingleOrMultiple, getDistance, getDistances, getHslAnimationFromHsl, getLinkColor, getLinkRandomColor, getLogger, getRandom, getRangeMax, getRangeValue, getStyleFromHsl, getStyleFromRgb, hslToRgb, hslaToRgba, initParticleNumericAnimationValue, isArray, isDivModeEnabled, isInArray, isNull, isObject, isPointInside, isSsr, itemFromArray, itemFromSingleOrMultiple, loadFont, millisecondsToSeconds, mouseLeaveEvent, mouseMoveEvent, parseAlpha, percentDenominator, randomInRange, rangeColorToHsl, rangeColorToRgb, rectBounce, rgbToHsl, setRangeValue, updateAnimation, updateColor } from "./browser-DyR09nk6.js";
	...
	constructor(container, engine) {
		...
		this._clickBubble = () => {
			...
			for (const particle of query) {
				...
				const pos = particle.getPosition(), distMouse = getDistance(pos, mouseClickPos), timeSpent = ((/* @__PURE__ */ new Date()).getTime() - (container$1.interactivity.mouse.clickTime ?? defaultClickTime)) / millisecondsToSeconds;
...
var Pusher = class extends ExternalInteractorBase {
	constructor(container) {
		...
		this.handleClickMode = (mode) => {
			...
			const group = itemFromArray([void 0, ...pushOptions.groups]), groupOptions = group !== void 0 ? container$1.actualOptions.particles.groups[group] : void 0, particlesOptions = itemFromSingleOrMultiple(pushOptions.particles), overrideOptions = deepExtend(groupOptions, particlesOptions);
	...
	constructor(container, engine) {
		this._drawLinkLine = (p1, link) => {
			...
			container$1.canvas.draw((ctx) => {
				...
				if (!colorLine) {
					const linkColor = p1LinksOptions.id !== void 0 ? container$1.particles.linksColors.get(p1LinksOptions.id) : container$1.particles.linksColor;
		...
		this._drawLinkTriangle = (p1, link1, link2) => {
			...
			container$1.canvas.draw((ctx) => {
				...
				if (!colorTriangle) {
					const linkColor = linksOptions.id !== void 0 ? container$1.particles.linksColors.get(linksOptions.id) : container$1.particles.linksColor;
