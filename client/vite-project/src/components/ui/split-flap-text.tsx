...
export interface SplitFlapTextProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
  words?: string[];
  text?: string;
  flipDuration?: number;
  stagger?: number;
...
const resolveCharset = (charset?: string) => {
  if (charset && CHARSETS[charset as keyof typeof CHARSETS]) return CHARSETS[charset as keyof typeof CHARSETS];
...
export default function SplitFlapText({
  ...
  // Start with empty space tiles so the initial mount animates into the first phrase
  const [tiles, setTiles] = useState<Tile[]>(() => createTiles(' '.repeat(width)));
  ...
  useEffect(() => {
    ...
    currentTextRef.current = ' '.repeat(width);
    ...
    const animateTo = (targetPhrase: string, force = false) => {
      ...
      const plans = targetChars
        .map((targetChar, index) => {
          ...
          if (fromChar === targetChar && (!force || targetChar === ' ')) return null;
    ...
    const initialDuration = animateTo(firstPhrase);
    ...
    const scheduleNext = (delay: number) => {
      cycleTimerRef.current = window.setTimeout(() => {
        ...
        if (normalizedPhrases.length <= 1) {
          if (!loop) return;
          const animDuration = animateTo(normalizedPhrases[0] || '', true);
  ...
  const componentStyle = {
    ...
    '--flap-tile': tileColor,
    '--flap-text': textColor,
    '--flap-radius': toCssUnit(tileRadius),
    '--flap-gap': toCssUnit(gap),
    '--flap-size': toCssUnit(fontSize),
    '--flap-speed': `${Math.max(0.04, Number(flipDuration) || 0.12)}s`,
    ...style
  } as CSSProperties;
